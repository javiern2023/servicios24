...
public class servidorHilo extends Thread{
			...
			try {
				...
				switch(opcion) {
					...
					case 4: //Recojo el número de líneas
						...
						break;
					case 5: System.out.println("El cliente ha solicitado su archivo de números.");

					    byte[] contenidoFichero = ficheroNumeroCliente(f);
					    
					    // Enviar primero el tamaño del archivo al cliente
					    out.writeInt(contenidoFichero.length);
					    System.out.println("Tamaño del archivo enviado al cliente: " + contenidoFichero.length);
	
					    // Enviar los bytes solo si el archivo no está vacío
					    if (contenidoFichero.length > 0) {
					        out.write(contenidoFichero);
					        System.out.println("Archivo enviado correctamente al cliente.");
					    } else {
					        System.out.println("El cliente no tiene números almacenados.");
					    }
					    break;
	...
	public int numeroLineasFicheroCliente(File f) throws IOException {
		...
		while((linea = br.readLine()) != null) {
			String[] partes = linea.split(":");
			if(partes.length > 1 && partes[1].trim().equals(nombreCliente.trim())) {
				numLineas++;
			}
		}
		br.close();
