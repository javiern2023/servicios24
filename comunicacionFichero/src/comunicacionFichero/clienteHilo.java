			...
			try {
				...
				switch(opcion) {
					...
					case 4: int numLineasCliente = in.readInt();
						...
						break;
					case 5: 
					    int tamañoFichero = in.readInt(); // Recibir el tamaño del archivo en bytes
					    System.out.println("Este cliente tiene " + tamañoFichero + " números almacenados."); 
					    
					    if (tamañoFichero > 0) {
					        byte[] contenidoFichero = new byte[tamañoFichero];
					        in.readFully(contenidoFichero); // Leer todos los bytes exactos
					        ...
					        System.out.println("Archivo 'numeros_cliente.txt' guardado con éxito.");
					    } else {
					        System.out.println("No hay números almacenados para este cliente.");
					    }
					    break;
					case 6: salir = true;
						break;
